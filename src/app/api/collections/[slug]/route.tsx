...
// Structure provided by Next.js for route parameters
...
// --- API Route Handler ---
export async function GET(request: Request, { params }: RouteParams): Promise<NextResponse<ApiResponse>> {
    // 1. Check Credentials
    if (!apiKey || !clientId) {
    ...
    try {
        ...
        if (!data.response || !Array.isArray(data.response.results)) {
            console.warn(`Invalid data structure received for slug "${slug}" (Page: ${page}). Expected response.results array. URL: ${apiUrl.substring(0, 200)}...`);
            return NextResponse.json({
        ...
        console.error(`Error in API route handler for slug "${slug}" (Page: ${page}):`, error);
