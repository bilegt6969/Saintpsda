...
const Home = () => {
  ...
  // Effect 1: Fetch collections list (checks cache first)
  useEffect(() => {
    // Use cache if valid
    if (isCacheValid(homeCache.timestamp) && homeCache.collections) {
      console.log("Using cached collections.");
      setCollections(homeCache.collections); // Set state from cache
      // Don't set isLoading here, wait for other data checks
      return; // Exit if cache is valid
    }

    // Fetch collections if not cached or expired
    console.log("Fetching collections...");
    setIsLoading(true); // Indicate loading only if fetching this critical piece
    setError(null);
    fetch("/api/payload/collections?sort=order")
      ...
      .then((fetchedData) => {
        if (!fetchedData || !Array.isArray(fetchedData.docs)) {
          throw new Error("Invalid response format received for collections");
        }
        const fetchedCollections = fetchedData.docs as CollectionDoc[];
        setCollections(fetchedCollections); // Update state
        homeCache.collections = fetchedCollections; // Update cache
        homeCache.timestamp = Date.now(); // Update timestamp
        console.log("Collections fetched and cached.");
      })
      .catch((error) => {
        console.error("Error fetching collection URLs:", error);
        setError(`Failed to load collections list: ${(error as Error).message}.`); // Use type assertion
        setIsLoading(false); // Stop loading on error for this part
      });
  }, []); // Run once on mount
