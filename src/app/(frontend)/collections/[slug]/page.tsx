...
export default function CollectionPage({ params }: { params: { slug: string } }) {
  ...
  const fetchProducts = useCallback(
    ...
    [collectionSlug]
  ); // Dependency: Refetch if slug changes
  ...
  // --- Initial Data Load Effect (on slug change) ---
  useEffect(() => {
    // Trigger only when collectionSlug is available and changes
    if (collectionSlug) {
      ...
      console.error('Collection slug is missing on mount!');
    ...
    // This effect runs when collectionSlug changes
  }, [collectionSlug, fetchProducts, fetchCurrencyData]); // Include stable fetch functions

  // --- Fetch More Products Effect (on page change) ---
  useEffect(() => {
